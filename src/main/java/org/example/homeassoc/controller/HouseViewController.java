...
import org.example.homeassoc.entity.Account;
import org.example.homeassoc.entity.House;
...
public class HouseViewController {
    ...
    @FXML
    private TableColumn<House, Integer> column1;
    @FXML
    private TableColumn<House, Integer> column2;
    @FXML
    private TableColumn<House, Integer> column3;

    @FXML
    private TableView<House> view;
    ...
    public void clickCreateButton() throws SQLException {
        try {
            ...
            fxmlLoader.setLocation(Main.class.getResource("CreateHouse.fxml"));
            Scene scene = new Scene(fxmlLoader.load());
            Stage stage = new Stage();
            stage.setTitle("Create House");
    ...
    public void clickUpdateButton() throws SQLException {
        House selectedReceipt = view.getSelectionModel().getSelectedItem();

        if (selectedReceipt != null) {
            String sql = "DELETE FROM public.\"House\" WHERE \"House_ID\" = ?";
            Connection connection = DatabaseConnection.getInstance();

            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, selectedReceipt.getHouse_ID());
        ...
        try {
            ...
            Scene scene = new Scene(fxmlLoader.load());
            Stage stage = new Stage();
            stage.setTitle("Create CreateAccount For Service");
            stage.setScene(scene);
            stage.show();
        } catch (IOException e) {
            Logger logger = Logger.getLogger(getClass().getName());
    ...
    public void clickDeleteAllButton() throws SQLException {
        ...
        if (selectedReceipt != null) {
            String sql = "DELETE FROM public.\"House\"";
            Connection connection = DatabaseConnection.getInstance();
    ...
    public void initialize() throws SQLException {
        ...
        column1.setCellValueFactory(new PropertyValueFactory<House, Integer>("House_ID"));
        column2.setCellValueFactory(new PropertyValueFactory<House, Integer>("Address"));
        column3.setCellValueFactory(new PropertyValueFactory<House, Integer>("Number_of_apartments"));
        view.setItems(getAll());
    }



    ObservableList<House> getAll() throws SQLException {
        Connection conn = DatabaseConnection.getInstance();

        Statement st = conn.createStatement();
        ResultSet rs = st.executeQuery("SELECT * FROM \"House\"");
        ObservableList<House> persons = FXCollections.observableArrayList();

        while (rs.next()) {
            persons.add(new House(rs.getInt(1), rs.getString(2), rs.getInt(3)));
        }
        rs.close();
        st.close();
