...
import org.example.homeassoc.entity.Service;
...
public class ServiceViewController {
    @FXML
    private TableColumn<Service, Integer> column1;
    @FXML
    private TableColumn<Service, String> column2;
    @FXML
    private TableColumn<Service, Integer> column3;
    @FXML
    private TableColumn<Service, Date> column4;
    @FXML
    private TableColumn<Service, Date> column5;

    @FXML
    private TableView<Service> view;
    ...
    public void clickCreateButton() throws SQLException {
        try {
            ...
            fxmlLoader.setLocation(Main.class.getResource("CreateService.fxml"));
    ...
    public void clickUpdateButton() throws SQLException {
        Service selectedReceipt = view.getSelectionModel().getSelectedItem();

        if (selectedReceipt != null) {
            String sql = "DELETE FROM public.\"Service\" WHERE \"Service_ID\" = ?";
            Connection connection = DatabaseConnection.getInstance();

            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, selectedReceipt.getService_ID());
    ...
    public void clickDeleteAllButton() throws SQLException {
        ...
        if (selectedReceipt != null) {
            String sql = "DELETE FROM public.\"Service\"";
            Connection connection = DatabaseConnection.getInstance();
    ...
    public void initialize() throws SQLException {
        ...
        column1.setCellValueFactory(new PropertyValueFactory<Service, Integer>("Service_ID"));
        column2.setCellValueFactory(new PropertyValueFactory<Service, String>("Name"));
        column3.setCellValueFactory(new PropertyValueFactory<Service, Integer>("Tariff"));
        column4.setCellValueFactory(new PropertyValueFactory<Service, Date>("Date_of_appearance"));
        column5.setCellValueFactory(new PropertyValueFactory<Service, Date>("Termination_date"));
        view.setItems(getAll());
    }



    ObservableList<Service> getAll() throws SQLException {
        Connection conn = DatabaseConnection.getInstance();

        Statement st = conn.createStatement();
        ResultSet rs = st.executeQuery("SELECT * FROM \"Service\"");
        ObservableList<Service> persons = FXCollections.observableArrayList();
...
//            System.out.println(rs.getString(6));
            persons.add(new Service(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getDate(4), rs.getDate(5)));
        }
        rs.close();
        st.close();
